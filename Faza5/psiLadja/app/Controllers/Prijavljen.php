...
class Prijavljen extends BaseController
{
    ...
    public function plati()
    {
        ...
        if(!empty($telefon))
        {
            if (!preg_match('/^(\+381|0)6\d{7,8}$/', $telefon)) {
            // Vraćanje greške i prekid izvršavanja metode
            return $this->prikaz('placanjePreuzimanje', ['poruka' => 'Molimo Vas unesite ispravan broj telefona, pocinje sa +3816 ili 06 i posle toga 7 ili 8 cifara.']);
        }
        }
         
        if(!empty($brojRacuna))
        {
            if (!preg_match('/^\d{3}-\d{13}-\d{2}$/', $brojRacuna)) {
            // Vraćanje greške i prekid izvršavanja metode
            return $this->prikaz('placanjePreuzimanje', ['poruka' => 'Molimo Vas unesite ispravan broj racuna, XXX-XXXXXXXXXXXXX-XX .']);
        }
        }
